...
{
    return ((p) == 2 ? 0 : ((p) == 3 ? 1 : ((p) >= 18 && (p) <= 21 ? 23 - (p) : -1)));
}
...
Adafruit_VS1053_FilePlayer::Adafruit_VS1053_FilePlayer(int8_t mosi, int8_t miso, int8_t clk,
    int8_t rst, int8_t cs, int8_t dcs, int8_t dreq, int8_t cardcs)
...
Adafruit_VS1053::Adafruit_VS1053(
    int8_t mosi, int8_t miso, int8_t clk, int8_t rst, int8_t cs, int8_t dcs, int8_t dreq)
{
    _mosi = mosi;
    _miso = miso;
    _clk = clk;
...
uint16_t Adafruit_VS1053::loadPlugin(char* plugname)
{
    ...
    if ((plugin.read() != 'P') || (plugin.read() != '&') || (plugin.read() != 'H'))
        return 0xFFFF;
...
void Adafruit_VS1053::startRecordOgg(boolean mic)
{
    ...
    if (mic) {
        ...
        sciWrite(
            VS1053_REG_MODE, VS1053_MODE_SM_LINE1 | VS1053_MODE_SM_ADPCM | VS1053_MODE_SM_SDINEW);
    }
