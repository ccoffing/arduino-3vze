...
#define SSPI_WRITE16(s)                                                                            \
    SSPI_WRITE((s) >> 8);                                                                          \
    SSPI_WRITE(s)
#define SSPI_WRITE32(l)                                                                            \
    SSPI_WRITE((l) >> 24);                                                                         \
    SSPI_WRITE((l) >> 16);                                                                         \
    SSPI_WRITE((l) >> 8);                                                                          \
    SSPI_WRITE(l)
#define SSPI_WRITE_PIXELS(c, l)                                                                    \
    for (uint32_t i = 0; i < (l); i += 2) {                                                        \
        SSPI_WRITE(((uint8_t*)(c))[i + 1]);                                                        \
        SSPI_WRITE(((uint8_t*)(c))[i]);                                                            \
    }
...
#define HSPI_BEGIN_TRANSACTION()                                                                   \
    SPI_OBJECT.beginTransaction(SPISettings(_freq, MSBFIRST, SPI_MODE0))
...
#define SPI_WRITE_PIXELS(c, l)                                                                     \
    if (_sclk < 0) {                                                                               \
        HSPI_WRITE_PIXELS(c, l);                                                                   \
    } else {                                                                                       \
        SSPI_WRITE_PIXELS(c, l);                                                                   \
    }
